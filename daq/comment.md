...
# организация билда, пакет системы

Наверно для "robust"-ness нужно чтобы билд собирал 1 директорию полностью готовую к использованию.
Т.е. чтобы её можно было скопировать и сразу работать в ней как в рабочем окружении системы.
Так чтобы и много пользователей могли иметь свои рабочие директории, с чёткой фиксацией версий.

Например:
билд проверяет пуст ли индекс гита,
собирает все пакеты в директорию,
название которой включает текущий комит (можно ещё последний тег),
сохраняет инфу о билде и сообщение последнего коммита в .build файле (или директории),
копирует ридми и стандартные конфиги если есть какие-то конфиги.

А что если во время операций нужно что-то тестить и менять в скрипте?
Перепаковывать всю систему тогда? Как учитывать такие изменения?

## локальные и центральные (системные) логи

Возможна идея локальной директории с логами для рабочей директории системы.
Но тогда зачем syslog и центральное хранение логов?

Именно для поддержания общей системы, общего проекта.
То есть на практике нужно делать вести отчёты для общего е-лога что и когда запускалось, что происходило.
Но суть в том что это не отдельные бессвязные рабочие директории,
а один проект, одна группа людей работает над 1 системой, ДАК установлена на 1 компе.
Важно организовать и поддерживать такие центральные узлы для эффективной кооперации народа.

Так же нужно организовать центральный репозиторий кода,
определить у кого там пуш-права, а когда надо делать пулы.

И центральное хранилище документации и инфы.
Стоит ли её хранить вместе с кодом? Или это по смыслу отдельная штука?
В принципе полезно иметь какие-то датакарды вместе с кодом, про инструкции бибиотек и т.д.
